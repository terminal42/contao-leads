...
class LeadDatabase
{
	...
	protected function __construct()
	{
		...
		$strMode = 'top';
		$resFile = fopen(TL_ROOT . '/system/modules/leads/config/database.sql', 'rb');
		...
		while (!feof($resFile))
		{
			...
			if ($strTrim == '-- LEAD FIELDS START --')
			{
				$strMode = 'data';
				continue;
			}

			if ($strTrim == '-- LEAD FIELDS STOP --')
			{
				$strMode = 'bottom';
				continue;
			}
	...
	public function storeFile()
	{
		...
		$objFile = new File('system/modules/leads/config/database.sql');
	...
	public static function getInstance()
	{
		...
		{
			self::$objInstance = new LeadDatabase();
		}
